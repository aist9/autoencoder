...
class VAE():
    def __init__(self, input_shape, hidden, act_func=F.tanh, use_BN=False, folder='./model', init_method='xavier'):
        ...
        if act_func != F.tanh and init_method=='xavier':
            init_method = chainer.initializers.HeNormal
            print("act_func is not recomended xavier initializer. Changed xavier -> HeNormal.")
        ...
        self.encoder = Encoder(hidden, act_func, use_BN, init_method)
        self.decoder = Decoder(hidden, act_func, use_BN, init_method)
    ...
    def featuremap_to_image(self, feat):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            d_out = self.decoder(Variable(feat))
