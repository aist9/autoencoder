# デコーダがmuとvarの2出力であるVAE(出力にGauss分布を仮定したVAEと言われるっぽい). 1出力のものと以下の点が異なる.
# ・ロス関数に gaussian_nll(negative log likelihood) を使う. 1出力verでは bernoulli_nll.
# ・入力データを0~1にする必要がない. ->  bernoulli_nll では第二引数に sigmoid が適用されるが, こちらではそれがない.
# ・ロスがマイナスの値になる. おそらく正常な動作だが, -infに発散することがある.

# ファイル名の _ed はエンコーダとデコーダが分かれていることを示す. optimizerが2つ作られるので学習に影響があると思われる.
# 実装したはいいが, 再構成データがどれに相当するかわからない（おそらくデコーダのmu?）
# 隠れ層ごとに何の活性化関数を使用するかは設定できない

# これを実装した最大の理由は「非正則化異常度を用いた工業製品の異常検知」の手法を実験するため.
# 論文中の D_VAE/A_VAE/M_VAEを出力するため, ロス関数をgaussian_nllではなく直接実装している(結果が変わらないことは確認済み).
# VAEクラスの reconstメソッドの引数 "unregular" をTrueにすると上記手法のロス(A_VAE)を出力する.
...
class Encoder(chainer.Chain):
    ...
    def makeLayers(self, hidden):
        ...
        # self.add_link('enc_mu' ,L.Linear(hidden[-2],hidden[-1], initialW=chainer.initializers.HeNormal()))
        # self.add_link('enc_var',L.Linear(hidden[-2],hidden[-1], initialW=chainer.initializers.HeNormal()))
...
class Decoder(chainer.Chain):
    ...
    def makeLayers(self,hidden):
        for i in range(len(hidden)-2):
            ...
            # l = L.Linear(hidden[j],hidden[j-1], initialW=chainer.initializers.HeNormal())
            name  = 'dec{}'.format(i)
            self.add_link(name,l)
        ...
        # self.add_link('dec_mu' ,L.Linear(hidden[1],hidden[0], initialW=chainer.initializers.HeNormal()))
        # self.add_link('dec_var',L.Linear(hidden[1],hidden[0], initialW=chainer.initializers.HeNormal()))
...
class VAE():
    def __init__(self, input_shape, hidden, act_func=F.tanh, use_BN=False, folder='./model'):
        ...
        self.encoder = Encoder(hidden, act_func, use_BN)
        self.decoder = Decoder(hidden, act_func, use_BN)
    ...
    def train(self,train,epoch,batch,C=1.0,k=1, gpu_num=0,valid=None, is_plot_weight=False):
        ...
        # print('epoch\tloss\t\tlatent\t\treconst')
        ...
        for ep in range(epoch):
            ...
            for p in range(0,nlen,batch):
                ...
                mu, ln_var = self.encoder(encoder_input)
                d_vae = 0.5 * F.sum( -ln_var -1  +mu*mu +F.exp(ln_var) , axis=1) * C
                # latent_loss = C * F.gaussian_kl_divergence(mu, ln_var) / btc # ↑の式と等価

                loss = 0
                for l in range(k):
                    decoder_input = F.gaussian(mu, ln_var)
                    d_mu, d_var = self.decoder(decoder_input)

                    # reconst_loss = F.gaussian_nll(encoder_input, d_mu, d_var) / (k*btc) # a_vae + m_vaeと等価
                ...
                # 逆伝搬 
                self.cleargrads() # encoder/decoderを同時にcleargradするメソッド
                loss.backward()
                self.update()     # encoder/decoderを同時に更新するメソッド
        ...
        self.to_cpu() # encoder/decoderを同時にto_cpuするメソッド
    ...
    def plot_weight(self, epoch):
        ...
        layer_num = len( list(filter(lambda x: not('BN' in x.name), self.encoder.children() )) )
        
        j = 1
        fig = plt.figure(figsize=(16,8))
        for i in range(2):
            Layers = self.encoder.children() if i == 0 else self.decoder.children()
            for layer in Layers:
                if 'BN' in layer.name:
                    continue
    ...
    def reconst(self, data, unregular=False):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            e_mu,e_var = self.encoder(Variable(data))
            feat = F.gaussian(e_mu, e_var).data
            d_mu,d_var = self.decoder(feat)
    ...
    def featuremap_to_image(self, feat):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            d_mu,d_var = self.decoder(Variable(feat))
